...
function actualizar(button) {
  if ((resultado.value.search(/[\+\-\*\//]/) != "-1" && (button.innerText == "+" || button.innerText == "/" || button.innerText == "*" || button.innerText == "-"))) {
    return;
  };

  if (resultado.value == 0 && (button.innerText == "+" || button.innerText == "/" || button.innerText == "*")) {
    console.log("a")
    return;
  };

  if (resultado.value == 0 && button.innerText != '-') {
    resultado.value = '';
  };
...
function calcular() {
  resultado.value = eval(resultado.value); //.toLocaleString("en-US"); //.toLocaleString('de-DE')
};
